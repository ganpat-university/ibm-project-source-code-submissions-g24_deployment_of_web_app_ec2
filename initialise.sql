...
DELETE FROM booked_tickets where show_id in (select show_id from shows where datediff(Date,cast(now() as date))<0 or (datediff(Date,cast(now() as date))<=0 and timediff(cast(convert_tz(cast(time*100 as time),'+00:00','+00:30') as time),cast(now() as time))<0));

DELETE FROM cancelled_tickets where show_id in (select show_id from shows where datediff(Date,cast(now() as date))<0 or (datediff(Date,cast(now() as date))<=0 and timediff(cast(convert_tz(cast(time*100 as time),'+00:00','+00:30') as time),cast(now() as time))<0));

DELETE FROM shows where datediff(Date,cast(now() as date))<0 or (datediff(Date,cast(now() as date))<=0 and timediff(cast(convert_tz(cast(time*100 as time),'+00:00','+00:30') as time),cast(now() as time))<0);
...
update completed_tickets set status="Complete" where status="Booked" and datediff(Date,cast(now() as date))<0 or (datediff(Date,cast(now() as date))<=0 and timediff(cast(convert_tz(cast(time*100 as time),'+00:00','+00:30') as time),cast(now() as time))<0);
